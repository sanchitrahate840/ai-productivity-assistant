...
app = FastAPI(title="AI Productivity Assistant Service", version="2.0.0")
...
async def chat(payload: ChatRequest, x_service_token: str | None = Header(default=None)):
    ...
    async with httpx.AsyncClient(timeout=45) as client:
        response = await client.post("https://api.openai.com/v1/responses", headers={"Authorization": f"Bearer {api_key}", "Content-Type": "application/json"}, json={"model": model, "input": prompt})
